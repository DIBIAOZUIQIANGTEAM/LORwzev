...
import com.wsns.lor.adapter.PickPictureAdapter;
...
public class PickPictureActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mGroupId = intent.getLongExtra(LorApplication.GROUP_ID, 0);
        if (mGroupId != 0) {
            mIsGroup = true;
            mConv = JMessageClient.getGroupConversation(mGroupId);
        } else {
            mTargetId = intent.getStringExtra(LorApplication.TARGET_ID);
            mTargetAppKey = intent.getStringExtra(LorApplication.TARGET_APP_KEY);
    ...
    private OnItemClickListener onItemListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> viewAdapter, View view, int position, long id) {
            ...
            intent.putExtra(LorApplication.POSITION, position);
            ...
            startActivityForResult(intent, LorApplication.REQUEST_CODE_BROWSER_PICTURE);
        }
    };
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == LorApplication.RESULT_CODE_SELECT_PICTURE) {
            if (data != null) {
        ...
        } else if (resultCode == LorApplication.RESULT_CODE_BROWSER_PICTURE) {
            setResult(LorApplication.RESULT_CODE_SELECT_ALBUM, data);
            finish();
        }
    }
            ...
            if (activity != null) {
                switch (msg.what) {
                    case SEND_PICTURE:
                        ...
                        activity.setResult(LorApplication.RESULT_CODE_SELECT_ALBUM, intent);
