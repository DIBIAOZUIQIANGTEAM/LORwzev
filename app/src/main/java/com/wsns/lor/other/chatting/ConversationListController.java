...
import com.wsns.lor.adapter.ConversationListAdapter;
...
import com.wsns.lor.utils.chat.SortConvList;
import com.wsns.lor.view.ConversationListView;
...
public class ConversationListController implements OnClickListener,
    ...
    public ConversationListController(ConversationListView listView, ConversationListFragment context,
        ...
        this.mWidth = width;
        initConvListAdapter();
        mConvListView.setRefreshListener(new ConversationListView.RefreshListener() {
            @Override
    ...
    public void onItemClick(AdapterView<?> viewAdapter, View view, int position, long id) {
        ...
        if (position > 0) {
            ...
            intent.putExtra(LorApplication.CONV_TITLE, conv.getTitle());
            if (null != conv) {
                // 当前点击的会话是否为群组
                if (conv.getType() == ConversationType.group) {
                    long groupId = ((GroupInfo) conv.getTargetInfo()).getGroupID();
                    intent.putExtra(LorApplication.GROUP_ID, groupId);
                    intent.putExtra(LorApplication.DRAFT, getAdapter().getDraft(conv.getId()));
                    ...
                    intent.putExtra(LorApplication.TARGET_APP_KEY, conv.getTargetAppKey());
